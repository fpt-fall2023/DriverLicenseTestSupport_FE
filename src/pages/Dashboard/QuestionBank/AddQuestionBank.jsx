...
const AddQuestionBank = () => {
  ...
  const handleQuestionCategory = () => {
    ...
    getQuestions().then((res) => {
      if (res.status === 200 || res.status === 304) {
        ...
        const khaiNiem = data.filter(
          (item) =>
            item.category.questionType ===
            'khái niệm và quy tắc giao thông đường bộ',
        );
        const nghiepVuVanTai = data.filter(
          (item) => item.category.questionType === 'nghiệp vụ vận tải',
        );
        const vanHoaDaoDuc = data.filter(
          (item) =>
            item.category.questionType === 'văn hóa & đạo đức người lái xe',
        );
        const kyThuatLaiXe = data.filter(
          (item) => item.category.questionType === 'kỹ thuật lái xe',
        );
        const cauTaoSuaChua = data.filter(
          (item) => item.category.questionType === 'cấu tạo sữa chữa',
        );
        const bienBao = data.filter(
          (item) =>
            item.category.questionType === 'hệ thống biển báo hiệu đường bộ',
        );
        const saHinh = data.filter(
          (item) =>
            item.category.questionType ===
            'sa hình và kỹ năng xử lý tình huống giao thông',
        );
        const diemLiet = data.filter(
          (item) =>
            item.category.questionType ===
  ...
  const handleQuestionToBankLogic = (
    question,
    startWith,
    arrayIndex,
    questionLimit,
    ...
    if (question.category.questionType.startsWith(`${startWith}`)) {
      ...
      const checkDuplicate = updatedQuestionToBank[arrayIndex].question.find(
      ...
      if (
        updatedQuestionToBank[arrayIndex].question.length >
        questionLimit - 1
      ) {
  ...
  const handleSubmitQuestionBank = () => {
    const allQuestions = questionToBank.reduce(
      (questionBank, currentQuestionCategory) => {
        return questionBank.concat(currentQuestionCategory.question);
      },
      [],
    );
    addQuestionBank(questionBankName, allQuestions, driveType)
      .then((res) => {
        if (res.status === 201) {
          notification.success({
    ...
    <div style={{ height: '100%', padding: '30px' }}>
      ...
      <Spin spinning={isLoading} indicator={tohruLoading}>
        <div className={styles.questionBank__details}>
          ...
          <div
            className={styles.questionBank__details__description}
            id="secondPart"
          >
            ...
            >
              ...
              <div id="questionDescription_name">
                {questionShow?.questionName}
              </div>
              <img
                src={questionShow?.questionImage}
                style={{ width: '20rem' }}
              />
              ...
              <Button
                type="primary"
                style={{ width: '10rem', height: '3rem', marginTop: '1rem' }}
                onClick={() => handleAddQuestionToBank(questionShow)}
              >
                Thêm
              </Button>
            </div>

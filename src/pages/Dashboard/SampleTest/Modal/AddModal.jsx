...
const AddModal = ({ isOpen, setIsOpen, questionBank, getAllSampleTest }) => {
    ...
    const onFinish = (values) => {
        addSampleTest(values.sampleTestName, values.questionBankName, driveType).then((res) => {
            if (res.status === 201) {
                setIsOpen(false)
            ...
            console.log(err)
            notification.error({
                message: 'Thêm thất bại. Hãy chắc rằng tên không trùng với bài thi thử khác!'
            })
        })
    }
                        ...
                        <Select placeholder="Chọn ngân hàng đề">
                            {questionBank.map((item) => (
                                <Select.Option key={item._id} value={item._id}>
                                    <div onClick={() => setDriveType(item.driveType)}>{item.questionBankName} - {item.driveType}</div>
                                </Select.Option>
                            ))}
                        </Select>
                    </Form.Item>
